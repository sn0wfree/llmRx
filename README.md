...
P0 + P1 + P2 + P3（Session TTL + 日志过滤 + Analytics + L3 策略可调 + 自动化 web-sync）已完成。
...
- ✅ **L3 cost 策略运行时切换**（cheapest/fastest/balanced）— Settings 页
- ✅ **自动化 web-sync**：`make build` 检测到前端变更自动 rebuild + 重新 embed
...
- ✅ 测试：76 个用例 8 包 race-clean，总覆盖率 64.5%
