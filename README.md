...
## 当前阶段：P7+ 闭环 + 多租户 + 热重载 + 配置网页化 ✅

P0 → P7+ 全部完成，所有 L1-L5 路由层齐全，多协议 provider，运行时配置，SSE 实时日志，告警，Docker，多租户强制，热重载，配置可网页化（3-PR + Effective Tab）。
...
镜像 ≈ 13 MB（`FROM scratch` + 静态链接 CGO 二进制 + ca-certs + 2 行 passwd），
启动 / 健康检查 / master-key bootstrap / privilege drop / bind-mount chown
全部由 `llmRx` 二进制自己完成 —— 没有 shell、没有 busybox、没有 entrypoint。

```bash
# 推荐：build 与 compose 拆开，避免受限网络下 buildkitd 不读 mirror 配置
make docker-build          # 主机编译 Go → docker build（daemon 路径，命中 mirror）
make docker-run            # docker compose up -d（用 llmrx:local 镜像，不重建）

# 公网环境可一步到位
docker compose up -d --build
```

入口在 `http://localhost:8787/admin/`，首次会自动生成 `LLMRX_KEY_MASTER`
并写入 `/data/llmrx.key`（mode 0600，属 llmrx 用户）。生产环境请通过
`LLMRX_KEY_MASTER` 环境变量或 docker secret 显式注入。

**Troubleshooting**

- `docker compose up --build` 报 `registry-1.docker.io: i/o timeout`：buildkitd
  不读 daemon 的 mirror 配置。`make docker-build && make docker-run` 绕开
  buildkitd；或在 `~/.docker/buildkit/buildkitd.toml` 加：
  ```toml
  [registry."docker.io"]
  mirrors = ["<your mirror>"]
  ```
...
| [docs/AUTO-ROUTING.md](docs/AUTO-ROUTING.md) | 智能路由 + 自我进化学习三阶段规划（v1 已实施） |
