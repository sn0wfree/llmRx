...
func TestLimiter_RPMAllowsThenBlocks(t *testing.T) {
	l := New()
	for i := 0; i < 3; i++ {
		ok, reason := l.Allow(1, 3, 0, 0, 0, 0, 0)
		if !ok {
...
func TestLimiter_TPMRejectsByTokenCount(t *testing.T) {
	l := New()
	ok, _ := l.Allow(1, 0, 100, 60, 0, 0, 0)
	if !ok {
		t.Fatal("first request should pass under tpm=100")
	}
	ok, reason := l.Allow(1, 0, 100, 60, 0, 0, 0)
	if ok {
...
func TestLimiter_BudgetExceededRejected(t *testing.T) {
	l := New()
	ok, reason := l.Allow(1, 0, 0, 0, 5.0, 4.5, 1.0)
	if ok || reason != "budget exceeded" {
		t.Fatalf("expected budget exceeded, got ok=%v reason=%q", ok, reason)
	}
	ok, reason = l.Allow(1, 0, 0, 0, 5.0, 3.0, 1.0)
	if !ok {
		t.Fatalf("within-budget request should pass, got reason=%q", reason)
	}
	ok, _ = l.Allow(1, 0, 0, 0, 0, 100.0, 1.0) // unlimited budget
	if !ok {
		t.Fatal("budgetUSD=0 should mean unlimited")
	}
}

func TestLimiter_AccountAddsToLastBucket(t *testing.T) {
	l := New()
	l.Allow(1, 0, 100, 10, 0, 0, 0)
	// Account 50 completion tokens to the same key; total is now 60,
	// still under the 100 cap.
	ok, _ := l.Allow(2, 0, 100, 0, 0, 0, 0) // different key
	if !ok {
...
func TestLimiter_WindowSlides(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		l.Allow(1, 2, 0, 0, 0, 0, 0)
	}
	if ok, _ := l.Allow(1, 2, 0, 0, 0, 0, 0); ok {
...
func TestLimiter_ResetAndKeyIsolation(t *testing.T) {
	l := New()
	l.Allow(1, 1, 0, 0, 0, 0, 0)
	l.Allow(2, 1, 0, 0, 0, 0, 0)
	if got := l.TrackedKeys(); got != 2 {
		t.Fatalf("tracked: %d", got)
	}
	if ok, _ := l.Allow(1, 1, 0, 0, 0, 0, 0); ok {
		t.Fatal("key 1 over cap")
	}
	if ok, _ := l.Allow(2, 1, 0, 0, 0, 0, 0); ok {
...
func TestLimiter_ConcurrentSafe(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		...
		go func(id int) {
			defer wg.Done()
			for j := 0; j < 100; j++ {
				l.Allow(int64(id%10), 0, 0, 1, 0, 0, 0)
