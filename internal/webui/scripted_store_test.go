...
var errNotImplemented = errors.New("not implemented in ScriptedStore")

// ScriptedStore is a test-only store that delegates to an underlying
// store.Store but allows per-method error injection via function fields.
// Defined locally in the webui package to avoid import cycles with testhelper.
...
func (s *ScriptedStore) IncrementPlanSpend(planID int64, amount float64) error {
	if s.IncrementPlanSpendFunc != nil { return s.IncrementPlanSpendFunc(planID, amount) }
...
func (s *ScriptedStore) TopByModel(f store.LogFilter, limit int) ([]store.NamedMetric, error) {
	if s.TopByModelFunc != nil { return s.TopByModelFunc(f, limit) }
...
func (s *ScriptedStore) TopByChannel(f store.LogFilter, limit int) ([]store.NamedMetric, error) {
	if s.TopByChannelFunc != nil { return s.TopByChannelFunc(f, limit) }
...
func (s *ScriptedStore) SetRuntimeSettings(payload []byte) error {
	if s.SetRuntimeSettingsFunc != nil { return s.SetRuntimeSettingsFunc(payload) }
...
func (s *ScriptedStore) ListBYOKChannels(ctx context.Context) ([]*model.BYOKChannel, error) {
	if s.ListBYOKChannelsFunc != nil { return s.ListBYOKChannelsFunc(ctx) }
